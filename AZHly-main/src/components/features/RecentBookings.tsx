  ...
  return (
    <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <h3 className="text-base font-semibold">Recent Classrooms</h3>
        {showViewAll && (
          <button
            onClick={onViewAll}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              {["Room ID", "Department", "Room", "Floor", "Block", "Status", ""].map((col) => (
          ...
          <tbody>
            {bookings.map((b, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 whitespace-nowrap">{b.department}</td>
                <td className="px-4 py-3 whitespace-nowrap">{b.room}</td>
                <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{b.floor}</td>
                <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{b.block}</td>
